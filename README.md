...
Currently, variants allow you to have distinct walls, lights, sounds, tiles, regions, and backgrounds for each variant. V14 levels support also exists, but all variants have the same levels. Each variant can have different level configurations, but not additional or removed levels. Scene name, navigation name and environment are also saved per variant.
